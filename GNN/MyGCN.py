...
out_dir = "mygnn_result"

data_size = 493  # 事件对pair的数量，实际为正对（same）的数量（因为正对比负对少）
train_size = int(data_size * 0.6)  # 训练集数目，取60%
node_size = 493  # 节点数目
node_embedding = 20  # 节点特征向量长度（tf-idf)

# train
class_size = 199  # 分类数量
meta_size = 20  # 元路径数量
epoch_num = 40000  # 迭代周期数
...
def get_data():
    ...
    train_label = label[index]  # diff和same各随机取了data_size个样本
    test_index = [i for i in range(data_size) if i not in index]
...
class PGCN(object):
    ...
    def forward_propagation(self):
        ...
        with tf.variable_scope('spectral_gcn'):
            gcn_out = GCN(tf.expand_dims(self.x, 0), weighted_adj, [self.class_size]).build()
...
if __name__ == "__main__":
    ...
    with tf.Session() as sess:
        net = PGCN(class_size=class_size, meta=meta_size, nodes=node_size,
        ...
        for epoch in range(epoch_num):  # 神经网络执行次数
                ...
                if acc > max_acc:
                    ...
                    print('weight:', w)
                    net.saver.save(sess, "mygnn_result/gnn_model")
                    print('********************* Model Saved *********************')
                ...
                '''
                with open("mygcn_result/PRAF.csv", "w") as f:
                    writer = csv.writer(f)
